        ...
        if (Item != NULL) {
            ...
            if (tab.mappa_nodi.find(posint) != tab.mappa_nodi.end()) {
                ...
                if ((!ClientData.DatiAgv[Item->Tag].load) || (posdep > 0)
                    ...
                    CentroMissione.posdep = posdep;
                    pianoprel = dmExtraFunction->Constrain(dmDB->PrimoPianoOk(CentroMissione.posprel) - 1, 1, 10);
...
void TdmGestMissioni::MoveMissioneManClick(TMenuItem *Item) {
    // missione manuale con numero posizione da input manuale
    int agv;
    AnsiString posman;
    int posint, posdep;
    ...
    if (!ClientData.ParametriFunzionali.AbilitaMissioni) {
        if (Application->MessageBox(L"Missioni disabilitate. Vuoi creare ugualmente la missione? ", L"Conferma", MB_YESNO) == IDNO)
            esegui = false;
    }
    if (esegui) {
        if (Item != NULL) {
            posman = InputBox(L"Imposta destinazione", L"Esegui", L"0");
            posint = posman.ToIntDef(0);
            if (tab.mappa_nodi.find(posint) != tab.mappa_nodi.end()) {
                posdep = dmDB->ControllaDepositoOk(posint, 0, 1);
                if (!posdep)
                    posdep = dmDB->ControllaDepositoOk(posint, 1, 1);
                if ((!ClientData.DatiAgv[Item->Tag].load) || (posdep > 0)
                    || (Application->MessageBox(L"Unreachable Drop Position, Do you want to create the mission the same ? ", L"Conferma", MB_YESNO) == IDYES)) {
                    if (!posdep) { // se non è una posizione di prel/dep
                        posdep = posint;
                    }
                    MissioneMan.Agv = Item->Tag;
                    MissioneMan.posprel = ClientData.DatiAgv[Item->Tag].pos;
                    MissioneMan.posdep = posdep;
                    MissioneMan.piano_prel = dmDB->PrimoPianoOk(MissioneMan.posprel) - 1;
                    // sulle missioni di spostamento lo forzo a 1
                    MissioneMan.piano_dep = dmDB->PrimoPianoOk(MissioneMan.posdep);
                    MissioneMan.h_prel = dmDB->RitornaAltezzedaPosizione(MissioneMan.posprel, MissioneMan.piano_prel, "HPREL");
                    MissioneMan.h_prel = max(MissioneMan.h_prel, ALTEZZA_PRELIEVO);
                    MissioneMan.h_dep = dmDB->RitornaAltezzedaPosizione(MissioneMan.posdep, MissioneMan.piano_dep, "HDEP");
                    MissioneMan.h_dep = max(MissioneMan.h_dep, ALTEZZA_PRELIEVO);
                    MissioneMan.priorita = 9;
                    // MissioneMan.IDUDC = ClientData.DatiAgv[Item->Tag].DatiUDC[1].IDUDC;
                    // MissioneMan.IDUDC2 = ClientData.DatiAgv[Item->Tag].DatiUDC[2].IDUDC;
                    MissioneMan.tipo_mis = 4; // spostamento
                    MissioneMan.tipologia_mis = 4; // MISSIONE MANUALE
                    MissioneMan.tipologia_mov = 4; // spostamento
                    // MissioneMan.CorsiaDeposito = dmDB->ReturnNomeCorsiaDaPos(Item->Tag);
                    SocketDataModule->InviaMissioneClient(MissioneMan.posprel, MissioneMan.posdep, MissioneMan.tipo_mis, MissioneMan.Agv);

                }
            }
            else
                Application->MessageBox(L"Non trovo la posizione o non esiste", L"Missione non creata", MB_OK);
        }
    }
}
