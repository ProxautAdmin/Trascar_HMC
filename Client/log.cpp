...
void __fastcall TfLog::BitBtn2Click(TObject *Sender)
{
    ...
    if (Application->MessageBox(L"Vuoi storicizzare?", L"Conferma", MB_YESNO) == IDYES) {
        ...
        if (res > 0)
            Application->MessageBox(L"Storicizzazione Log eseguita", L"Ok", MB_OK);
        else
            Application->MessageBox(L"Problemi storicizzazione Log", L"Read log", MB_OK);
        FormActivate(Sender);
    }
}

// ---------------------------------------------------------------------------
void __fastcall TfLog::btEsportaClick(TObject *Sender)
{
    if (Tot->Caption.ToIntDef(0) > 0)
        dmExtraFunction->EsportaExcel(StringaExcel, "Log");
}
// ---------------------------------------------------------------------------
