...
//	cout << "Liberazione nodo " << id << " ok." << endl;
    if ((tab.mappa_nodi.find(id) == tab.mappa_nodi.end())||(agv <=0) ||(agv > NAGV)) {
...
void DATILGV::Load1(char *name) {
	...
	while (!feof(f)) {
		fscanf(f, "%s", s);
			...
			nd.nodocreato = 0 ;
			for (i = 0; i <= NAGV; i++) {
				nd.occupato_da_vicino.push_back(0);
			}
			mappa_nodi[n] = nd ;
...
void DATILGV::Setvicini() {
	...
	if (NAGV <= 1)// se ho solo un agv non importa calcolare i nodi vicini
        return ;
	filename = File_name;
