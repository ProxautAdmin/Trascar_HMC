    ...
    if ((PLCThread[Plc.id_thread_plc]->connected) || (ClientData.ParametriFunzionali.Simula)) {
        ...
        if (MainForm->count_inizio > 20) {
            if (Deposito.uscitedascrivere[0] != Deposito.uscite[0]) {
                ...
                dmDB->LogMsg("Modificato statouscite deposito" + IntToStr(Deposito.pos) + " su Plc pos " + IntToStr(Deposito.pos) + " , valore prec : " + IntToStr(Deposito.uscite[0]) + " , nuovo : " + IntToStr(Deposito.uscitedascrivere[0]));
            }
            if (Deposito.uscitedascrivere[1] != Deposito.uscite[1])
            {
                ...
                dmDB->LogMsg("Modificato idpallet deposito" + IntToStr(Deposito.pos) + " su Plc pos " + IntToStr(Deposito.pos) + " , valore prec : " + IntToStr(Deposito.uscite[1]) + " , nuovo : " + IntToStr(Deposito.uscitedascrivere[1]));
            }
            if (Plc.forzature_attive) {
                if (Plc.count_forzature > 30) {
                    Plc.count_forzature = 0;
...
void TDMPlc::ControlloIngressoPrelievi(DatiPlc &Plc, DatiPrelievo &Prelievo) {
    ...
    if (dmDB->lettostatoagv) {
        for (int agv = 1; agv <= NAGV; agv++) {
            if ((ClientData.DatiAgv[agv].richiestaconsenso) && (!ClientData.DatiAgv[agv].consensodato) && (ClientData.DatiAgv[agv].pos == Prelievo.pos_fuori_ingombro) && (MainForm->strcons[agv] == "xingresso")) {
...
void TDMPlc::ControlloIngressoDepositi(DatiPlc &Plc, DatiDeposito &Deposito) {
    ...
    if (dmDB->lettostatoagv) {
        for (int agv = 1; agv <= NAGV; agv++) {
            if ((ClientData.DatiAgv[agv].richiestaconsenso) && (!ClientData.DatiAgv[agv].consensodato) && (MainForm->strcons[agv] == "ingresso")) {
        ...
        if ((!ClientData.ParametriFunzionali.Simula) && (Plc.PlcConnesso)) {
            if ((ClientData.DatiAgv[a].pos == Prelievo.pos_fuori_ingombro) || (ClientData.DatiAgv[a].pos == Prelievo.pos)) {
                if ((!Prelievo.InAllarme) && (Prelievo.Ready)) {
                    if (((ClientData.DatiAgv[a].dest == Prelievo.pos) || (ClientData.DatiAgv[a].pos == Prelievo.pos_fuori_ingombro)) && (!Prelievo.RichiestaAbilitaPrel)) {
                    ...
                    else if (((ClientData.DatiAgv[a].dest == Prelievo.pos) || (ClientData.DatiAgv[a].pos == Prelievo.pos_fuori_ingombro)) && (!Prelievo.AGVInIngombro)) {
                ...
                if (((Prelievo.InAllarme) || (!Prelievo.Ready)) && ((ClientData.DatiAgv[a].dest == Prelievo.pos) || (ClientData.DatiAgv[a].pos == Prelievo.pos_fuori_ingombro))
        ...
        if ((!ClientData.ParametriFunzionali.Simula) && (Plc.PlcConnesso)) {
            if ((ClientData.DatiAgv[a].pos == Deposito.pos_fuori_ingombro) || (ClientData.DatiAgv[a].pos == Deposito.pos)) {
                if ((!Deposito.InAllarme) && (Deposito.Ready)) {
                    if (((ClientData.DatiAgv[a].dest == Deposito.pos) || (ClientData.DatiAgv[a].pos == Deposito.pos_fuori_ingombro)) && (!Deposito.RichiestaAbilitaDep)) {
                    ...
                    else if ((ClientData.DatiAgv[a].dest == Deposito.pos) && (Deposito.OkDeposito) && (!Deposito.AGVInIngombro)) {
                        // rulli in moto
                ...
                if ((ClientData.DatiAgv[a].pos == Deposito.pos) && (ClientData.DatiAgv[a].dest == Deposito.pos)
                    && ((!Deposito.OkDeposito) || (Deposito.InAllarme) || (!Deposito.Ready))) {
                    if (!ClientData.DatiAgv[a].stand_by) {
                        // fermo agv durante carico per allarme plc a terra
                        dmComandiAgv->StandByAgv(a, 1);
