...
void TDMGestione_Porte::LeggiStrutturaDatiPorte() {
    ...
    try {
        ...
        while (!ADOQuery->Eof) {
            ...
            ClientData.DatiPorte[id].DW_Addr_PortaReady = ADOQuery->FieldByName("DW_Addr_PortaReady")->AsInteger;
            ...
            ClientData.DatiPorte[id].Bit_PortaReady = ADOQuery->FieldByName("Bit_PortaReady")->AsInteger;
            ...
            ClientData.DatiPorte[id].DW_Addr_IngombroAGV = ADOQuery->FieldByName("DW_Addr_IngombroAGV")->AsInteger;
            ClientData.DatiPorte[id].DW_Addr_AGVInZona = ADOQuery->FieldByName("DW_Addr_AGVInZona")->AsInteger;
            ...
            ClientData.DatiPorte[id].Bit_IngombroAGV = ADOQuery->FieldByName("Bit_IngombroAGV")->AsInteger;
            ClientData.DatiPorte[id].Bit_AGVInZona = ADOQuery->FieldByName("Bit_AGVInZona")->AsInteger;
        ...
        if ((ClientData.DatiAgv[agv].richiestaconsenso) && (!ClientData.DatiAgv[agv].consensodato)) {
            try {
                ...
                if (porta) {
                    ...
                    /*
                     senza il controllo se ho il segnale di chiusura alto
                     if ((ClientData.DatiPorte[porta].Stato == 1) &&
                     ((!ClientData.DatiPorte[porta].PortaAperta) || (!ClientData.DatiPorte[porta].ApriPorta)) // && (ClientData.DatiPorte[porta].PortaChiusa > 0)
                    ...
                    if ((!ClientData.DatiPorte[porta].ApriPorta) && (ClientData.DatiPorte[porta].Stato == 1) &&
                    ...
                    else if (((res) || (ClientData.DatiPorte[porta].Stato >= 2)) &&
                        ...
                        if (ClientData.DatiPorte[porta].memoria_apertura_operatore)
                            MainForm->MessageBar1->Messages->Strings[ClientData.DatiAgv[agv].num_allarme_consenso_porta] = "Agv in attesa chiusura porta " + IntToStr(porta) + " dopo apertura manuale da operatore !!!";
                        else
...
int TDMGestione_Porte::ControllaPorteAperte() {
    ...
    for (i = 1; i <= NUM_PORTE; i++) {
        if (((ClientData.DatiPorte[i].PlcConnesso) || (ClientData.ParametriFunzionali.Simula)) && (!ClientData.DatiPorte[i].forzature_attive)) {
            if ((ClientData.DatiPorte[i].PortaAperta) || (ClientData.DatiPorte[i].ApriPorta) || (ClientData.DatiPorte[i].PortaTFuoco)) {
                res = AgvInPosPorta(i);
...
void TDMGestione_Porte::CompilaStrutturaPlcPorte(TPorte & StructPlcPorte) {
    ...
    if ((PLCPhoenixThread[StructPlcPorte.id_thread_plc]->connesso) || (ClientData.ParametriFunzionali.Simula)) {
        ...
        if ((StructPlcPorte.Bit_PortaReady >= 0) && (StructPlcPorte.DW_Addr_PortaReady >= 0)) {
            StructPlcPorte.PortaReady = PLCPhoenixThread[StructPlcPorte.id_thread_plc]->IngressiPhoenix[StructPlcPorte.DW_Addr_PortaReady] & dmDB->bit[StructPlcPorte.Bit_PortaReady];
        }
        ...
        if ((StructPlcPorte.Bit_IngombroAGV >= 0) && (StructPlcPorte.DW_Addr_IngombroAGV >= 0)) {
            StructPlcPorte.IngombroAGV = PLCPhoenixThread[StructPlcPorte.id_thread_plc]->UscitePhoenix[StructPlcPorte.DW_Addr_IngombroAGV] & dmDB->bit[StructPlcPorte.Bit_IngombroAGV];
        }
        if ((StructPlcPorte.Bit_AGVInZona >= 0) && (StructPlcPorte.DW_Addr_AGVInZona >= 0)) {
            StructPlcPorte.AGVInZona = PLCPhoenixThread[StructPlcPorte.id_thread_plc]->UscitePhoenix[StructPlcPorte.DW_Addr_AGVInZona] & dmDB->bit[StructPlcPorte.Bit_AGVInZona];
        }
