...
//      }
//---------------------------------------------------------------------------
Phoenix_Thread *PLCPhoenixThread[3];
...
{
	device = id ;
	//NameThreadForDebugging(((AnsiString)"PLCPhoenixThread" + IntToStr(id-1)), ThreadID);
	bit[0] = 1;
    bit[1] = 2;
    bit[2] = 4;
    bit[3] = 8;
...
// con add 0 alzo le uscite 5.1 - 6.1 - 5.4. - 6.4 - 7.1 - 8.1 - 7.4 - 8.4
        ...
        Write.length = 0x08 ;  // lunghezza byte da leggere
        ...
        if (alza)  // alzo l'uscita o l'abbasso
			UscitePhoenix[add] = UscitePhoenix[add] | bit[Bit] ;
        else
			UscitePhoenix[add] = ((UscitePhoenix[add]) & bitAnd[Bit] );
		if (connesso) {
			devoscrivere = true ; // forse conviene scrivere nel thread
			Write.data = UscitePhoenix ;
			//ddiRet = DDI_DTI_WriteData (dtiHnd, &Write);
		}else if (ClientData.ParametriFunzionali.Simula) {
		   /*	MainForm->DatiPhoenix[device].output[1] = UscitePhoenix[1] ;
			MainForm->DatiPhoenix[device].output[2] = UscitePhoenix[3] ;
			MainForm->DatiPhoenix[device].output[3] = UscitePhoenix[5] ;
			MainForm->DatiPhoenix[device].output[4] = UscitePhoenix[7] ;*/
		}
		if (ddiRet != ERR_OK) {
			return 0;
		}else {
